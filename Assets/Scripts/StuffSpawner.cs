...
public class StuffSpawner : MonoBehaviour
{
    ...
    void Start()
    {
        //let's decide whether we'll place an obstacle
        //Random.Range is exclusive for integers but inclusive for 
        bool placeObstacle = Random.Range(0, 1) == 0; //50% chances
        int obstacleIndex = -1;
        if (placeObstacle)
        {
        ...
        for (int i = 0; i < StuffSpawnPoints.Length; i++)
        {
            //don't instantiate if there's an power ups
            if (i == obstacleIndex) continue;
            if (Random.Range(0, 3) == 0) //33% chances to create powerups
            {
